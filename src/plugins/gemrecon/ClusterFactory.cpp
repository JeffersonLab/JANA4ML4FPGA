    ...
    void ClusterFactory::InitHistForZeroSup() {
        ...
        if ((fRunType == "SINGLEEVENT") || (fRunType == "SEARCHEVENTS")) {
            ...
            for (plane_itr = listOfPlanes.begin(); plane_itr != listOfPlanes.end(); ++plane_itr) {
                ...
                Int_t planeId =
                        2 * fMapping->GetDetectorID(fMapping->GetDetectorFromPlane(plane)) + (*plane_itr).second;
                ...
                f2DSingleEventHist[id] = new TH2F(presZeroSup, presZeroSup, m_srs_ntsamples, 0, driftTime, nbOfStrips,
                                                  0,
                                                  (nbOfStrips - 1));
        ...
        if (fRunType == "MULTIEVENTS") {
            ...
            for (plane_itr = listOfPlanes.begin(); plane_itr != listOfPlanes.end(); ++plane_itr) {
                ...
                fClusterHist[id] = new TH1F(clusterUniformity, clusterUniformity, nbOfStrips, -1 * size / 2.,
                                            size / 2.);
            ...
            for (det_itr = listOfDetectors.begin(); det_itr != listOfDetectors.end(); ++det_itr) {
              ...
              nbin2 = 25 ;
                ...
                */
                ...
                f2DPlotsHist[3 * detId + 1] = new TH2F(adcMap, adcMap, nbin1, -1 * size1, size1, nbin2, -1 * size2,
                                                       size2);
                ...
                f2DPlotsHist[3 * detId + 2] = new TH2F(timingMap, timingMap, nbin1, -1 * size1, size1, nbin2,
                                                       -1 * size2,
                                                       size2);
                ...
                fChargeSharingHist[detId] = new TH2F(chSharing, chSharing, 50, m_min_adc, m_max_adc, 50, m_min_adc,
                                                     m_max_adc);
...
// This function is called every event
    void ClusterFactory::Process(const std::shared_ptr<const JEvent> &event) {
        ...
        try {
            ...
            GEMOnlineHitDecoder hit_decoder(event->GetEventNumber(), m_srs_ntsamples, fStartTimeSample,
                                            m_srs_ntsamples - 1,
...
//
            ...
            if (ncx == ncy) {
                ...
                for (int ic = 0; ic < ncx; ic++) {
                    m_log->debug("  cl={}  x={}  E={} A={} N={}", ic, clustX[ic].x, clustX[ic].E, clustX[ic].A,
                                 clustX[ic].N);
                    m_log->debug("  cl={}  y={}  E=}| A={} N={}", ic, clustY[ic].x, clustY[ic].E, clustY[ic].A,
                                 clustY[ic].N);
...
//    m_log->trace("ClusterFactory finished\n");
    ...
    void ClusterFactory::FillTrdHistogram(uint64_t event_number, TDirectory *hists_dir,
                                          std::vector<const DGEMSRSWindowRawData *> srs_data, int max_x, int max_y) {

        // check srs data is valid. At least assertions of what we have
        assert(srs_data.size() > 0);
        assert(srs_data[0]->samples.size() > 0);
        assert(srs_data[0]->samples.size() ==
               srs_data[srs_data.size() - 1]->samples.size());  // At least some test of samples size consistency
        ...
        for (auto apvPair: event_map) {
            ...
            for (auto channel_pair: apv_channels) {
                ...
                // go over samples
                for (size_t sample_i = 0; sample_i < samples.size(); sample_i++) {
                    int bin_index = sample_i * 128 + sample_i + fMapping->APVchannelCorrection(
                            apv_channel);  // + sample_i to make gaps in between samples
    ...
    void ClusterFactory::TraceDumpMapping(std::vector<const DGEMSRSWindowRawData *> srs_data, size_t print_rows) {
        ...
        for (size_t i = 0; i < rows_to_display; i++) {
            ...
            std::string row = fmt::format("{:<7} {:<7} {:<9} {:<8} {:<13} {:<2} ",
                                          srs_item->rocid, srs_item->slot, srs_item->channel,
                                          srs_item->apv_id, srs_item->channel_apv,
                                          fMapping->APVchannelCorrection(srs_item->channel));
