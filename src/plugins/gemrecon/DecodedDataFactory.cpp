...
//
        ...
        std::map<int, std::vector<double>> offsets = pedestal->OffsetsByApv();
        std::map<int, std::vector<double>> noises = pedestal->NoisesByApv();

        hit_decoder.ProcessEvent(fec_apv_raw, offsets, noises);
        std::map<std::string, Int_t> listOfPlanes = fMapping->GetPlaneIDFromPlaneMap();
        std::map<std::string, Int_t>::const_iterator plane_itr;
        int pad = 0;
        int ncx = 0, ncy = 0;
