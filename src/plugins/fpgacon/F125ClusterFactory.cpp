...
namespace ml4fpga::fpgacon {
    ...
    void F125ClusterFactory::CozyProcess(uint64_t run_number, uint64_t event_number) {
		...
		for(int i=0; i < nclust; i++) {
			...
			cluster->id = i;
			cluster->pos_x = clust_Xpos[nclust];
			cluster->pos_y = clust_Ypos[nclust];
			cluster->pos_z = clust_Zpos[nclust];
			cluster->dedx = clust_dEdx[nclust];
			cluster->size = clust_Size[nclust];
			cluster->width[0] = clust_Width[nclust][0];
			cluster->width[1] = clust_Width[nclust][1];
			cluster->width[2] = clust_Width[nclust][2];
			cluster->length[0] = clust_Length[nclust][0];
			cluster->length[1] = clust_Length[nclust][1];
			cluster->length[2] = clust_Length[nclust][2];
		}

		m_cluster_output().push_back(new F125Cluster{1});
