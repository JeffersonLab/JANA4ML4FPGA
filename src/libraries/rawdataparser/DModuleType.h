...
public:
    ...
    // Constructor
    //
    DModuleType(type_id_t type, std::string name, std::string description) : type(type), name(name),
    ...
    static DModuleType GetModule(type_id_t id) {
        switch (id) {
            case TID      :
                return DModuleType(TID, "TID", "JLab Trigger Interface");
            case FADC250  :
                return DModuleType(FADC250, "FADC250", "JLab Flash 250 MHz ADC");
            case FADC125  :
                return DModuleType(FADC125, "FADC125", "JLab Flash 125 MHz ADC");
            case F1TDC32  :
                return DModuleType(F1TDC32, "F1TDC32", "JLab F1 TDC (32 chan)");
            case F1TDC48  :
                return DModuleType(F1TDC48, "F1TDC48", "JLab F1 TDC (48 chan)");
            case JLAB_TS  :
                return DModuleType(JLAB_TS, "JLAB_TS", "JLab Trigger Supervisor");
            case TD       :
                return DModuleType(TD, "TD", "JLab Trigger Distribution");
            case SSP      :
                return DModuleType(SSP, "SSP", "JLab Sub-System Processor");
            case JLAB_DISC:
                return DModuleType(JLAB_DISC, "JLAB_DISC", "JLab Discriminator");

            case VMECPU   :
                return DModuleType(VMECPU, "VMECPU", "VME CPU");
            case CAEN1190 :
                return DModuleType(CAEN1190, "CAEN1190", "CAEN 1190 TDC");
            case CAEN1290 :
                return DModuleType(CAEN1290, "CAEN1290", "CAEN 1290 TDC");

            default:
                return DModuleType(UNKNOWN, "UNKNOWN", "Unknown module type");
        }
    }
    ...
    /// has the type.
    static std::string GetName(type_id_t id) {
    ...
    /// be called without an instance of the class if one already
    /// has the type.
    static std::string GetDescription(type_id_t id) {
