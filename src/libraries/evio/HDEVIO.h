...
class HDEVIO {
public:
    HDEVIO(std::string filename, bool read_map_file = true, int verbose = 1);
    ...
    public:
        ...
        uint32_t block_len;
        bool swap_needed;
        std::vector <EVIOEventRecord> evio_events;
        uint64_t first_event;
        uint64_t last_event;
    ...
    std::streampos last_event_pos; // used to hold file position at last event read in
    ...
    std::stringstream err_mess;  // last error message
    ...
    void SaveFileMap(std::string fname = "");

    void ReadFileMap(std::string fname = "", bool warn_if_not_found = false);

    uint32_t GetEventMask(void) { return event_type_mask; }

    uint32_t SetEventMask(uint32_t mask);

    uint32_t SetEventMask(std::string types_str);

    uint32_t AddToEventMask(std::string type_str);

    std::vector <EVIOBlockRecord> &GetEVIOBlockRecords(void);
...
protected:
    ...
    void SetErrorMessage(std::string mess) {
        ClearErrorMessage();
        err_mess << mess;
    }

    void buff_read(char *s, std::streamsize n);

    void buff_seekg(std::streamoff off, std::ios_base::seekdir way);

    std::streamsize buff_gcount() const { return (std::streamsize) _gcount; }
...
public:
    ...
    // HexStr
    //------------------------
    inline std::string HexStr(uint16_t v) {
        char str[256];
    ...
    // HexStr
    //------------------------
    inline std::string HexStr(uint8_t v) {
        char str[256];
