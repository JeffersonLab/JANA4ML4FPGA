...
struct SingleDimensionPeakData {
    ...
    SingleDimensionPeakData(int index, double height, int width, double area)
            : index(index), height(height), width(width), area(area) {}

    // Equality operator for comparing peaks based on their data
    bool operator==(const SingleDimensionPeakData& other) const {
